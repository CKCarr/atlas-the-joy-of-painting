...
export function convertCSVForPostgres(inputFilePath, outputFilePath) {
    ...
    // Remove single quotes around each element
    const finalProcessedData = processedData.replace(/'([^']*)'/g, '$1');

    // Remove the outer double quotes ("{ and }") while keeping the inner single quotes around each element
